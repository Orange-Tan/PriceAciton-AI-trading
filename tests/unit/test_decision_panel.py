...
def test_panel_has_no_collapsible_detail_buttons_and_embeds_future_trend(panel: DecisionPanel):
    assert not panel.findChildren(QToolButton)
    assert panel._future_trend_panel.parentWidget() is panel
    inner = {**_valid_no_order()["decision"]}
    inner["next_bar_prediction"] = {
        "probabilities": {"bullish": 60, "bearish": 20, "neutral": 20},
        "reasoning": "test",
        "unpredictable": False,
    }
    panel.set_prediction(inner)
    assert panel._future_trend_panel._bar_group.isVisible()
...
def test_panel_unpredictable_renders_gray(panel: DecisionPanel):
    ...
    assert "#6b7280" in panel._prediction_direction_label.styleSheet()
...
def test_panel_neutral_renders_yellow(panel: DecisionPanel):
    ...
    inner = {**data["decision"], "next_bar_prediction": data["next_bar_prediction"]}
    panel.set_decision(inner, diagnosis_summary=data.get("diagnosis_summary"))
    assert "中性的概率为55%" in panel._prediction_direction_label.text()
