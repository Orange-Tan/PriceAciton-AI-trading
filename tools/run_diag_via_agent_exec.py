...
params = json.dumps(
    {"sessionKey": "agent:main:main", "message": msg, "idempotencyKey": str(uuid.uuid4())}
)
subprocess.run(
    [
        node,
...
time.sleep(35)
p = sorted(
    Path.home().joinpath(".qclaw/agents/main/sessions").glob("*.jsonl"),
...
for line in p.read_text(encoding="utf-8").splitlines()[-30:]:
    ...
    if d.get("message", {}).get("role") == "toolResult":
        t = "".join(
            x.get("text", "") for x in d["message"].get("content", []) if isinstance(x, dict)
        )
